...
2. **GET_STATUS** `0x04` → `LEN=22` (`0x16`), разобрать телеметрию (см. [GET_STATUS](#get_status-004)).
3. После успешного старта MCU ожидается: `SCALER=ON`, `LCD=ON`, `BACKLIGHT=ON` при `ENABLE_BACKLIGHT_AUTO_STARTUP=1` (см. [6.1](#61-старт-системы)).
...
- [ ] PING → `0xAA`
- [ ] GET_STATUS → 22 байта DATA (`LEN=0x16`), CRC сходится
...
`v24, v12, v5, v3v3` (u16) → токи ×5 (i16, мА, знаковые) → `state` (u8) → `fault_flags` (u16) → `inputs` (u8).  
У нулевой нагрузки токи могут быть слегка отрицательными (шум, смещение калибровки) — парсить как `int16`, не `uint16`.  
`i_backlight = -32768`, если датчик тока BL не установлен ([19.2](#192-каналы-temp0temp1) — NTC/температура в UART не передаются).
...
3. **Неверный CRC / фрейминг / таймаут / случайный байт без STX** → NACK `CMD=0xFF` с `error_code` `0x03`–`0x07` (см. [§9](#9-протокол-uart)).
...
MCU ждёт `PGOOD`, затем сам включает `SCALER` + `LCD` + `BACKLIGHT` (при `ENABLE_BACKLIGHT_AUTO_STARTUP=1`, sequencing §13.2).  
Подробно: [6.1](#61-старт-системы), [13.6](#136-интеграция-с-запуском-системы-раздел-61).

Ожидаемая маска `state` (биты POWER_CTRL): `SCALER | LCD | BACKLIGHT` (`0x07`).

### Настроить яркость подсветки

Предусловие: после старта `BACKLIGHT` уже ON (или включить вручную через `POWER_CTRL`, если auto-BL отключён).

1. `SET_BRIGHTNESS`: `pwm` 0…1000.
2. При необходимости выключить/включить подсветку — `POWER_CTRL` bit2 ([13.7](#137-интеграция-с-power_ctrl-002)).
...
| GET_STATUS «короче 22»              | Неверный парсер LEN                 | `LEN` в кадре = `0x16` (22 DATA)                                                                             |
...
NTC на текущей ревизии **не установлены**; каналы Temp0/Temp1 в UART не передаются ([19.2](#192-каналы-temp0temp1)).
...
2. Устанавливает safe state:
   - домены питания в OFF (см. раздел 4.1: для active HIGH это LOW), **кроме `ETH1`/`ETH2`**, которые всегда удерживаются во включённом состоянии (см. ниже)
   - open-drain линии “отпущены” (release, не тянуть в LOW)
   - усилитель в `shutdown/mute` (`SDZ = LOW`, `MUTE = HIGH`)
3. Сразу включает `ETH1` + `ETH2` (`power_manager_init()`, безусловно, без ADC-верификации и без зависимости от `PGOOD`) — эти домены не секвенируются и не блокируются ревизией платы (см. [раздел «Домены ETH1 и ETH2»](#домены-eth1-и-eth2))
4. Ждёт `PGOOD = HIGH` (активный опрос; если не появился за 5 секунд — установить `FAULT_PGOOD_LOST` и оставаться в INIT)
5. Включает подсистемы по значениям по умолчанию:
   - `SCALER` + `LCD` + `BACKLIGHT` (при `ENABLE_BACKLIGHT_AUTO_STARTUP=1` и `ENABLE_BACKLIGHT_HW=1`) — **через полный display power sequencing** (раздел 13.2)
...
При любом подтверждённом fault `power_safe_state()` выключает все управляемые домены, кроме `ETH1`/`ETH2`. Эти два домена являются специальным инвариантом и не гасятся прошивкой ни при каких условиях.

Дальнейшее управление осуществляется командами от Q7.

#### Домены ETH1 и ETH2

- Всегда включены сразу после `power_manager_init()` (до ожидания `PGOOD`), физически — HIGH сразу при `MX_GPIO_Init()`.
- Команды `POWER_CTRL`, содержащие биты `ETH1`/`ETH2`, принимаются, но не меняют их физическое состояние: линии остаются HIGH, биты в `power_state` остаются установленными.
- Не участвуют в §6.5 auto-startup дисплея: наличие включённых `ETH1`/`ETH2` не блокирует и не задерживает автозапуск `SCALER`/`LCD`/`BACKLIGHT`.
- Не выключаются ни `power_safe_state()`, ни `power_force_off_domains()`, ни любым `POWER_CTRL`; `power_emergency_display_off()` (аварийное отключение только дисплея) их также не трогает.
...
- ожидание `STX`: получение `0x02` начинает новый пакет
- `CMD`/`LEN`/`DATA`/`CRC`: байт `0x02` внутри этих полей воспринимается как обычные данные, а не как ресинхронизация — границы кадра задаёт `LEN`, экранирования нет
- ожидание `ETX`: если байт `!= 0x03`, кадр отбрасывается; если этот байт при этом равен `0x02`, он не теряется, а сразу становится `CMD` нового кадра
...
- CRC error → NACK `error_code=0x03`
- неверный ETX / `LEN>64` → NACK `error_code=0x04`
- таймаут неполного кадра (межбайтовый 10 мс или пакетный 50 мс) → NACK `error_code=0x05`
- переполнение RX ring → NACK `error_code=0x06`
- неизвестный `CMD` → NACK `error_code=0x01`
- случайный байт без `STX` (шум на линии) → NACK `error_code=0x07`

Если байт одновременно завершает межбайтовый таймаут предыдущего кадра **и** сам не является `STX` — репортится только `0x05` (таймаут), не `0x07`: причина уже объяснена таймаутом, второй NACK на тот же байт не шлётся.
...
- `0x03` = CRC error
- `0x04` = framing error (неверный ETX, `LEN>64`)
- `0x05` = frame timeout (неполный кадр)
- `0x06` = RX ring overflow
- `0x07` = случайный байт без `STX` (шум на линии, а не попытка кадра)
...
- все управляемые домены питания выключены: `GET_STATUS.state == 0x30` (только биты `ETH1|ETH2`, `DOM_ETH1|DOM_ETH2`);
...
После `PGOOD` MCU сам выполняет шаги 1–6 ([13.2](#132-последовательность-включения-дисплея)), включая `BACKLIGHT` при `ENABLE_BACKLIGHT_AUTO_STARTUP=1`. Итог: [6.1](#61-старт-системы), [сценарий Q7](#после-подачи-питания-без-участия-q7).
...
- температурные каналы в `GET_STATUS` не передаются (резерв на плате)

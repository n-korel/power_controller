#!/usr/bin/env bash
# F.1 with display load: narrow V12 → FAULT_V12_RANGE (state!=0 from SCALER+LCD)
set -euo pipefail
...
v12="$(parse_get_status_hex "$gs" | awk -F= '/^v12=/{print $2}')"
log_info "trap V12 under load: v12=${v12} mV, thresholds ${FAULT_V12_TRAP_MIN_MV}..${FAULT_V12_TRAP_MAX_MV} mV"
...
log_pass "F.1 load: V12 range fault latched with display on, cleared"

...
- после reset в самом начале `main()` проверяется магик по фиксированному адресу SRAM, и при совпадении выполняется прыжок

**Нельзя** проверить, что после прыжка реально оживает системный загрузчик **STM32F030**: адрес `0x1FFF0000` у F0 и F4 содержит разный ROM‑bootloader с разным протоколом. То есть «вошли в bootloader ROM» можно проверить **только на целевом F030**. На F407 мы убеждаемся лишь в корректной работе механизма «магик → reset → детект → jump».
...
/test_f407/            ← отдельный каталог, отдельный .ioc, отдельный build
  ...
  Services/
    ...
    bootloader.c       ← логика магика+jump без реального перехода в ROM
  Protocol/            ← без изменений
  Config/
    config.h           ← те же пороги/константы, ADC_VREF_MV = 3000 (для F407), остальное как есть
  tests/
...
- [ ] `BOOTLOADER_ENTER`: ACK → TX complete → магик в SRAM → reset → детект → попытка jump (проверяется, что до jump доходим; сам ROM‑bootloader на F407 не проверяем)
...
Цель — воспроизвести 14‑канальный контракт основного проекта настолько, насколько возможно. На DISC1 нет всех 14 пинов, поэтому делаем **один ADC‑канал, но тот же контракт буфера**. Маппинг индексов (0..13) на один физический пин оставляем в коде тестов.
...
- `Analog → ADC1`:
  ...
  - `Parameters`:
    ...
    - `Number Of Conversion`: `1` (для простого loopback; если хотим имитировать буфер длиной 14 — см. ниже)
    - `Rank 1 → Channel 6`, `Sampling Time`: `480 Cycles` (самый длинный, аналогично основному проекту)

**Вариант «полный 14‑канальный буфер»**: если хочется один‑в‑один имитировать контракт основного проекта, добавить в регулярной группе 14 рангов **все на канал 6** (так CubeMX разрешает; либо выставить часть на `Vrefint`/`Temp sensor`/`Vbat` — они есть у F4). Тогда `adc_dma_buf[14]` заполняется полностью и тест фильтра/индексации идёт как на F030.
...
Переопределения **только** тех констант, где физика платы реально другая (всё остальное — идентично основному `config.h`):

```c
#define ADC_VREF_MV          3000u   /* на F407 VDDA ≈ 3.0 В, НЕ 2.5 В как на F030 */
#define ADC_CHANNEL_COUNT    14      /* контракт сохраняем, каналы 0..13 маппятся на IN6 в тесте */
```

Все остальные пороги (`THRESH_*`, `SEQ_*`, `FAULT_*`, `PROTO_*`, `CMD_*`) **не меняем** — это часть тестируемого контракта.

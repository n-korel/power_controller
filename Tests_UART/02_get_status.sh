#!/usr/bin/env bash
# Test_firmware C.2 — GET_STATUS, 31-byte frame, bare-board telemetry
set -euo pipefail
...
validate_get_status_hex "$hex" || die "invalid GET_STATUS frame (expected 31 bytes: STX CMD LEN=0x1A … CRC ETX)"
parse_get_status_hex "$hex"
if expect_get_status_clean "$hex"; then
  log_pass "GET_STATUS: 31 bytes, state=0, fault=0"
else

...
- [16. Требования к реализации](#16-требования-к-реализации)
...
Используется **IWDG (Independent Watchdog)**. Цель — перезапуск MCU при зависаниях/зацикливаниях и при “тихих” ошибках логики, когда main loop перестаёт делать полезную работу.

### 15.1 Где разрешено делать refresh

`HAL_IWDG_Refresh(&hiwdg)` (или ваш `watchdog_refresh()`) **вызывается ровно в одном месте**: в основном цикле, **один раз за итерацию**, и **только после** того, как выполнены критичные для “здоровья” шаги текущей итерации.

Важно для CubeMX: если в `.ioc` включён `IWDG` (`VP_IWDG_VS_IWDG.Mode=IWDG_Activate`), `MX_IWDG_Init()` запускает watchdog сразу во время старта, но CubeMX **не** генерирует `HAL_IWDG_Refresh(&hiwdg)` в main loop автоматически. Поэтому после каждой генерации кода нужно вручную проверить, что refresh добавлен и стоит **ровно в одной точке** main loop.

Рекомендованный порядок (пример):

```
while (1) {
    process_uart();      // RX/TX + разбор пакетов
    read_adc();          // обновление данных АЦП (DMA snapshot)
    update_state();      // state machine (sequencing/логика)
    handle_faults();     // фиксация fault + safe state
    watchdog_refresh();  // единственная точка refresh
}
```

Смысл: если любой из шагов выше завис/сломался/перестал вызываться — refresh не произойдёт и IWDG перезапустит MCU.

Если refresh не добавить после генерации, прошивка будет уходить в reset на каждом старте. Если же кормить watchdog из ISR/HAL callback, можно получить “живую” систему с зависшим main loop, что маскирует реальную неисправность.

### 15.2 Где refresh запрещён (чтобы не “маскировать” зависания)

Запрещено вызывать `HAL_IWDG_Refresh()`:

- в прерываниях (SysTick, DMA, UART IRQ и т.п.)
- в HAL callback'ах (`HAL_UART_*Callback`, `HAL_ADC_*Callback` и т.п.)
- внутри функций, которые могут быть вызваны “слишком часто” или рекурсивно (парсер UART, драйверы GPIO/ADC и т.п.)
- внутри любых блокирующих ожиданий/петель “подождать пока …” (в идеале таких ожиданий вообще не должно быть)
- в коде секвенса дисплея/аудио, если это не верхнеуровневый шаг main loop

Это критично: если refresh находится в ISR или в низкоуровневой функции, MCU может “жить” бесконечно даже при зависшем main loop (например, когда UART IRQ продолжает приходить и кормить IWDG).

### 15.3 Таймаут и частота вызова

- IWDG настраивается примерно на **~1 секунду** (см. `STM32CubeMXConfig.md`, раздел про IWDG).
- При нормальной работе refresh должен происходить заметно чаще таймаута (например, каждые 10–100 мс в зависимости от частоты итерации main loop).

### 15.4 Состояние FAULT

В состоянии `FAULT` main loop продолжает выполняться, поэтому IWDG обычно **продолжает** refresh (чтобы устройство не уходило в reset-петлю и могло отвечать по UART телеметрией/fault_flags).

Если нужен режим “в FAULT не кормить и гарантированно перезагрузиться”, это должно быть **явно** закреплено отдельным требованием (и тогда refresh отключается только на верхнем уровне main loop).

---

## 16. Требования к реализации

- без блокирующих delay
- использование state machine
- фильтрация входов
- проверка АЦП данных
- безопасные состояния при старте (всё выключено)

### Дискретные входы (debounce)
...
## 17.2 Минимальный тест‑план на железе (Definition of Done)

Этот чеклист — критерии “прошивка работает как задумано”.

### Базовый старт и safe state

- [ ] После подачи питания все домены в safe state (см. раздел 4.1), open‑drain линии “отпущены”, усилитель в `shutdown/mute`.
- [ ] Если `PGOOD` не стал HIGH за 5 секунд → остаёмся в INIT и фиксируем `FAULT_PGOOD_LOST`.
- [ ] При `PGOOD = HIGH` выполняется автосеквенс до состояния: `SCALER=ON`, `LCD=ON`, `BACKLIGHT=OFF`, `TOUCH=ON`, `AUDIO=ON` (усилитель остаётся в `shutdown/mute`, см. 13.6).
- [ ] При включённом `IWDG` после генерации CubeMX проверено, что `HAL_IWDG_Refresh(&hiwdg)` стоит ровно в одной точке main loop, после обработки UART/ADC/state/fault, и не вызывается из ISR/HAL callback.

### POWER_CTRL (0x02) и секвенсы дисплея

- [ ] `BACKLIGHT=ON` разрешён только при `SCALER=ON` и `LCD=ON`; иначе `status=1`, состояние не меняется.
- [ ] Запрос `SCALER=OFF` или `LCD=OFF` при `BACKLIGHT=ON` приводит к корректному shutdown sequencing (13.7) без “жёсткого” гашения.
- [ ] Падение `PGOOD` во время секвенса приводит к аварийному выключению `BACKLIGHT_OFF → LCD_OFF → RST_LOW → SCALER_OFF` и установке `FAULT_SEQ_ABORT` + `FAULT_PGOOD_LOST`.

### Мониторинг и защита

- [ ] Выход напряжений `+24/+12/+5/+3.3` за `min..max` (после подтверждения) устанавливает соответствующий `FAULT_Vxx_RANGE` и отключает домены по таблице 19.4.2.
- [ ] Превышение токового порога по каналу (после подтверждения) устанавливает соответствующий `FAULT_*` и отключает домены по 19.4.2.
- [ ] Fault защёлкивается; без `RESET_FAULT` повторное включение не происходит.

### SUS_S3# (автозапуск Linux)

- [ ] Если `PGOOD=HIGH` и `SUS_S3#=LOW` > 500 мс → импульс `PWRBTN#` LOW 150 мс, повторы не чаще 1 раза/5с.

### CALIBRATE_OFFSET (0x09)

- [ ] Команда сохраняет offset токовых каналов во Flash, данные валидируются CRC при старте.
- [ ] При невалидных данных (CRC/magic/version) используется дефолтный offset.

### BOOTLOADER_ENTER (0x08)

- [ ] Команда переводит систему в safe state, ставит SRAM magic, выполняет reset и гарантированно входит в ROM‑bootloader (`0x1FFF0000`) без аппаратного BOOT0.

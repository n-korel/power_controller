...
3. Отправить ACK (ответ с `status=0x00`) и дождаться завершения передачи UART.
4. Выполнить `NVIC_SystemReset()`.
...
- состояние системы
- флаги ошибок
- состояния входов (IN_x, PGOOD, Faultz)
...
Диапазон выхода датчика укладывается в 0–3.3 В, но **АЦП MCU ограничен `VDDA=2.5 В`**, поэтому значения выше 2500 мВ будут **клипироваться**.
...
По контракту при **любом** fault выполняется safe state: **все** домены питания `SCALER/LCD/BACKLIGHT/AUDIO/ETH1/ETH2/TOUCH` принудительно выключаются. Далее `fault_flags` фиксирует причину(ы), а повторное включение возможно только по явной команде от Linux после `RESET_FAULT`.

Таблица ниже — это “бит fault → смысл/причина” (какой бит(ы) защёлкнуть в `fault_flags`).

| Fault бит | Смысл / причина (когда устанавливается) |
| --------- | -------------------------------------- |
| `FAULT_SCALER`     | Авария, связанная со `SCALER`. |
| `FAULT_LCD`        | Авария, связанная с `LCD`. |
| `FAULT_BACKLIGHT`  | Авария, связанная с `BACKLIGHT`. |
| `FAULT_AUDIO`      | Авария аудио: перегрузка токов и/или fault усилителя. |
| `FAULT_ETH1`       | Авария домена `ETH1` (резерв/будущий мониторинг). |
| `FAULT_ETH2`       | Авария домена `ETH2` (резерв/будущий мониторинг). |
| `FAULT_TOUCH`      | Авария домена `TOUCH` (резерв/будущий мониторинг). |
| `FAULT_PGOOD_LOST` | Потеря `PGOOD`. |
| `FAULT_AMP_FAULTZ` | `Faultz` усилителя подтверждён как fault. |
| `FAULT_V24_RANGE`  | `+24V` вышло за пороги. |
| `FAULT_V12_RANGE`  | `+12V` вышло за пороги. |
| `FAULT_V5_RANGE`   | `+5V` вышло за пороги. |
| `FAULT_V3V3_RANGE` | `+3.3V` вышло за пороги. |
| `FAULT_SEQ_ABORT`  | Секвенс дисплея был принудительно прерван. |
| `FAULT_INTERNAL`   | Внутренняя ошибка логики, приведшая к защитному отключению. |

Дополнительные действия (вне доменных битов, фиксируем как поведение):

- так как при любом fault выполняется safe state (все домены OFF), отдельные “частичные” отключения доменов не применяются.
- если дисплей был включён, при переводе в safe state соблюдать ограничение “не выключать `LCD` или `SCALER` при включённом `BACKLIGHT`”: сначала `BACKLIGHT_OFF`, затем остальное (без блокирующих задержек, насколько возможно).
- при `FAULT_AUDIO`/`FAULT_AMP_FAULTZ` при переводе в safe state дополнительно перевести усилитель в безопасный режим: `MUTE=HIGH`, `SDZ=LOW`.
...
- после получения команды MCU:
  ...
  - отправляет ACK и дожидается завершения передачи UART
  - делает reset
...
По запросу от Linux контроллер передаёт телеметрию и состояние входов (IN_x, `PGOOD`, `Faultz`).
...
- `RESET_BRIDGE (0x06)`
  ...
  - Response: `LEN=1` (`status:uint8`)
- `SET_THRESHOLDS (0x07)`
  - Request: `LEN` переменный (mask + значения по mask; см. описание `SET_THRESHOLDS (0x07)` в разделе 9)
  - Response: `LEN=1` (`status:uint8`)
- `BOOTLOADER_ENTER (0x08)`
  - Request: `LEN=0`

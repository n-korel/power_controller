...
void test_dispatch_get_version_layout_13_bytes(void)
{
    /* Host builds use Config/version_gen.h fallback defaults. */
    uint8_t pkt[8];
    ...
    const uint8_t *d = &tx_buf[3];
    TEST_ASSERT_EQUAL_UINT8_ARRAY((const uint8_t *)"00000000", d, 8);
    TEST_ASSERT_EQUAL_UINT8(0, d[8]);
    TEST_ASSERT_EQUAL_UINT8(0, d[9]);
    TEST_ASSERT_EQUAL_UINT8(0, d[10]);
    TEST_ASSERT_EQUAL_UINT8(0, d[11]);
    TEST_ASSERT_EQUAL_UINT8(0, d[12]);
